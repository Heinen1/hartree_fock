...
from time import perf_counter # for timing the calls to the functions
import numpy as np
...
print(electron_electron_repulsion(molecule))
t1_end = perf_counter()

t2_start = perf_counter()
electron_electron_repulsion_s(molecule)
t2_end = perf_counter()

print("Recursive: ", t1_end - t1_start)
print("Non-Recursive: ", t2_end - t2_start)

print("Nuc-Nuc repulsion", nuclear_nuclear_repulsion_energy(atom_coordinates, Z))
print("Nuc-Nuc repulsion", nuclear_nuclear_repulsion_energy(atom_coordinates, ['H', 'H']))
...
for molecule_coordinate in molecule_coordinates:
    ...
    Vee = electron_electron_repulsion_s(molecule)
...
for molecule_coordinate in molecule_coordinates:
    H1a = atom2('H', molecule_coordinate[0])
    H2a = atom2('H', molecule_coordinate[1])
    molecule = H1a.bf + H2a.bf
    atom_coordinates = np.array([molecule_coordinate[0], molecule_coordinate[1]])
    Z = [1.0, 1.0]
    # compute SCF energy
    S = overlap_recursive(molecule)
    T = kinetic_recursive(molecule)
    Vne = nuclear_electron_recursive(molecule, atom_coordinates, ['H', 'H'])
    Vee = electron_electron_repulsion(molecule)
    Enn = nuclear_nuclear_repulsion_energy(atom_coordinates, ['H', 'H'])
    molecular_terms = [S, T, Vne, Vee]
    scf_parameters = [1e-5, 20, ]
    electronic_energy = scf_cycle(molecular_terms, scf_parameters, molecule)
    # compute total energy =SCF energy + E_NN
    total_energy = electronic_energy + Enn
    total_energies_rec.append(total_energy)

import matplotlib.pyplot as plt
plt.xlabel("Bond distance (A)")
plt.ylabel("Total energy (Ha)")
plt.plot(np.array(distances) * conversion, total_energies_rec)
